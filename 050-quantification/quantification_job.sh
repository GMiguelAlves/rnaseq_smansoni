...
#SBATCH --output=logs/quantification/tximport_%j.out
#SBATCH --error=logs/quantification/tximport_%j.err
#SBATCH --ntasks=1
...
resolve_step_dir() {
    if [[ -n "${STEP_DIR:-}" && -f "${STEP_DIR}/run_quantification.sh" ]]; then
        echo "$STEP_DIR"
        return 0
    fi

    if [[ -n "${SLURM_SUBMIT_DIR:-}" && -f "${SLURM_SUBMIT_DIR}/run_quantification.sh" ]]; then
    ...
    if [[ -f "${script_dir}/run_quantification.sh" ]]; then
        echo "$script_dir"
        return 0
    fi
...
STEP_DIR_PATH="$(resolve_step_dir)" || {
    echo "[ERRO] Nao foi possivel localizar a etapa 050."
...
mkdir -p "${STEP_DIR_PATH}/logs/quantification"
cd "$STEP_DIR_PATH"
...
bash "${STEP_DIR_PATH}/run_quantification.sh" "$@"
