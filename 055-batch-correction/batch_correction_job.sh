#!/bin/bash
#SBATCH --job-name=batch_correct
#SBATCH --output=logs/batch/batch_correct_%j.out
#SBATCH --error=logs/batch/batch_correct_%j.err
#SBATCH --ntasks=1
...
resolve_step_dir() {
    if [[ -n "${STEP_DIR:-}" && -f "${STEP_DIR}/run_batch_correction.sh" ]]; then
        echo "$STEP_DIR"
        return 0
    fi

    if [[ -n "${SLURM_SUBMIT_DIR:-}" && -f "${SLURM_SUBMIT_DIR}/run_batch_correction.sh" ]]; then
    ...
    if [[ -f "${script_dir}/run_batch_correction.sh" ]]; then
        echo "$script_dir"
        return 0
    fi
...
bash "${STEP_DIR_PATH}/run_batch_correction.sh" "$@"
